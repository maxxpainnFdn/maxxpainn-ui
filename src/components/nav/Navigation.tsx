      ...
      >
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex justify-between items-center h-20">
            ...
            {/* Desktop Right Side */}
            <div className="hidden screen-width-900:flex items-center gap-3">
              <a
          ...
          {isOpen && (
            <div className="md:hidden bg-gray-900/95 backdrop-blur-xl border-t-2 border-purple-500/30 rounded-b-2xl shadow-xl">
              <div className="py-4 space-y-2 px-2">
                {navItems.map((item) => (
                  <Link
                    key={item.name}
                    to={item.href}
                    className="block px-4 py-3 text-gray-300 hover:text-white hover:bg-gradient-to-r hover:from-purple-600/20 hover:to-pink-600/20 rounded-xl transition-all font-medium border border-transparent hover:border-purple-500/30"
                ...
                <div className="px-4 py-2">
                  <div className="text-gray-400 font-bold mb-2 text-sm uppercase tracking-wide">
                    Docs
                  </div>
                  <div className="pl-2 space-y-2">
                    {docsLinks.map((item, idx) => (
                      <Link
                        key={idx}
                        to={item.href}
                        className="block px-3 py-2 text-gray-400 hover:text-purple-400 rounded-lg transition-colors"
                        onClick={() => setIsOpen(false)}
                      >
                        {item.name}
                      </Link>
