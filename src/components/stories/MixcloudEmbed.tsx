...
function buildEmbedUrl(feedPath: string): string {
  return (
    `https://www.mixcloud.com/widget/iframe/?feed=${encodeURIComponent(feedPath)}` +
...
const BAR_COUNT = 60;
const MC_TEAL = "#52aad8";
const MC_PURPLE = "#7c4dff";
...
// ---------------------------------------------------------------------------
// Waveform — clip-path overlay approach for continuous progress
// ---------------------------------------------------------------------------
function Waveform({
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 px-5 py-3">
        ...
        <div
          className="flex-1 h-9 flex items-center cursor-pointer relative"
          onClick={(e) => {
        ...
        >
          ...
          <div className="flex items-center gap-[1.5px] w-full h-full">
            {barHeights.map((h, i) => (
              <div
                key={i}
                className="flex-1 min-w-[2px] rounded-[2px]"
                style={{ height: `${h * 100}%`, background: `${MC_TEAL}20` }}
              />
            ))}
          </div>
          {/* Played overlay (teal) — continuous clip-path */}
          <div
          ...
          >
            {barHeights.map((h, i) => (
              <div
                key={i}
                className="flex-1 min-w-[2px] rounded-[2px]"
                style={{ height: `${h * 100}%`, background: MC_TEAL }}
              />
            ))}
          </div>
          {/* Leading-edge overlay (purple) — thin sliver at playhead */}
          {playing && (
            <div
              className="absolute inset-0 flex items-center gap-[1.5px]"
              style={{
            ...
            >
              {barHeights.map((h, i) => (
                <div
                  key={i}
                  className="flex-1 min-w-[2px] rounded-[2px]"
                  style={{ height: `${h * 100}%`, background: MC_PURPLE }}
                />
              ))}
            </div>
          )}
...
export default function MixcloudEmbed({ url, className = "" }: MixcloudEmbedProps) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        setMeta({ title, uploader, thumbnail, trackUrl: url, feedPath: parsed.feedPath, type: parsed.type });
  ...
  // Init Widget API once meta is ready
  // FIX: Wait for the iframe's own `load` event before calling PlayerWidget.
  // Calling it too early (before the Mixcloud page inside the iframe is ready)
  // means widget.ready never resolves and play() silently fails.
  useEffect(() => {
    if (!meta || !iframeRef.current) return;
    let cancelled = false;

    const iframe = iframeRef.current;
    iframe.src = buildEmbedUrl(meta.feedPath);
    ...
    // FIX: If the iframe already has a src and is cached, `load` may have already
    // fired. Use a small timeout as a fallback to ensure we don't miss it.
  ...
  const handlePlay = async () => {
    ...
    try {
      ...
      // Some browsers block autoplay — the widget throws; ignore silently.
    }
  };
  ...
  const handleSeek = async (ratio: number) => {
    ...
    setProgress(ratio);
    setCurrentSecs(ratio * durationSecs);
  };
  ...
  return (
    ...
    >
      ...
      {/* Widget iframe — visible so the browser counts it as a user-interacted frame,
          but positioned off-screen so it doesn't take up layout space.
          NOTE: display:none or visibility:hidden prevents autoplay in most browsers. */}
      <iframe
        ref={iframeRef}
        title="Mixcloud player"
        allow="autoplay"
        style={{ position: "absolute", width: 1, height: 1, top: 0, left: 0, opacity: 0, pointerEvents: "none", border: "none" }}
      />
          ...
          <div className="flex items-center gap-4 px-5 pt-4 pb-4">
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              <button
                ...
                className={cn(
                  "action-btn w-12 h-12 rounded-full flex items-center justify-center border-none flex-shrink-0",
                  "transition-all duration-200",
                  widgetReady ? "cursor-pointer hover:scale-105 active:scale-[0.96]" : "cursor-not-allowed"
                )}
                style={{
                  background: widgetReady ? (playing ? MC_PURPLE : MC_TEAL) : "var(--maxx-dim, #5a5370)",
                  boxShadow: widgetReady && playing ? `0 0 20px ${MC_PURPLE}66` : "none",
                }}
              >
                {playing ? (
              ...
              <a
                ...
                onMouseEnter={(e) => ((e.currentTarget as HTMLElement).style.background = `${MC_TEAL}18`)}
